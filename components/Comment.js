...
export default function Comment({ comment, commentId, originalPostId }) {
  ...
  async function deleteComment() {
    if(window.confirm('コメントを消去してもよろしいですか？')){
      deleteDoc(doc(db, "posts", originalPostId, "comments", commentId));
    }
  }

  return (
    <div className="flex p-3 cursor-pointer border-b border-grey-200 pl-20">
      {/* user image */}
      <img className="h-11 w-11 rounded-full mr-4" src={comment?.userImage} alt="user-image" />
      ...
      <div className="flex-1">
        ...
        {/* post text */}
        <p className="text-grey-800 text-[15px] sm:text-[16px] mb-2">
          {comment?.comment}
        </p>
        ...
        <div className="flex justify-between text-grey-500 p-2">
          ...
          {session?.user.uid === comment?.userId && (
            <TrashIcon
              ...
              className="h-9 hoverEffect p-2 hover:text-red-500 hover:bg-red-100"
            />
          )}
          <div className="flex items-center ">
          {hasLiked ? (
            <HeartIconFilled onClick={likeComment} className="h-9 hoverEffect p-2 text-red-600 hover:bg-red-100" />
          ) : (
            <HeartIcon onClick={likeComment} className="h-9 hoverEffect p-2 hover:text-red-600 hover:bg-red-100" />
          )}
          {
            likes.length > 0 && (
