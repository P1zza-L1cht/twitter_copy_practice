...
export default function Post({ post, id }) {
  ...
  return (
    <div className="flex p-3 cursor-pointer border-b border-grey-200">
      {/* user image */}
      ...
      <div className="flex-1">
        ...
        {/* post text */}
        <p onClick={() => router.push(`/posts/${id}`)} className="text-grey-800 text-[15px] sm:text-[16px] mb-2">
          {post?.data()?.text}
        </p>
        ...
        <div className="flex justify-between text-grey-500 p-2">
          ...
          {hasLiked ? (
            <HeartIconFilled onClick={likePost} className="h-9 hoverEffect p-2 text-red-600 hover:bg-red-100" />
          ) : (
